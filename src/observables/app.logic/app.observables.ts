...
export function getChangeDifficultyObs(app: AppComponent): Observable<[Event, NodeListOf<HTMLSelectElement>]>{
    ...
    return merge(...changeEvent$).pipe(
        map((eventChange) => [eventChange, difficultySelects])
    );
}
...
export function getOppponentObs(app: AppComponent): Observable<Opponent> {
    let startClick$ = fromEvent(app.getElement(CLASS_NAMES.BUTTONS.START), "click");
    let findNewOpponentClick$ = fromEvent(app.getElement(CLASS_NAMES.BUTTONS.FIND_NEW_OPPONENT),"click");
    return merge(startClick$, findNewOpponentClick$)
        .pipe(
            switchMap(() => {
