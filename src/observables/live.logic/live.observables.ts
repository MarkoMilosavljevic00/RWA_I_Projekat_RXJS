...
import { CLASS_NAMES, INDEXES, RULES, TIME } from "../../utilities/constants";
...
export function getFightTickObs(
    live: LiveComponent
): Observable<{ round: number, secondsElapsed: number, roundChange: boolean}> {
...
export function getCheckForFinishObs(live: LiveComponent) {
    return interval(TIME.TICK_DURATION).pipe(
        ...
        tap(() => console.log("DESIO SE FINISH"))
    );
}
...
export function getFightCardLiveObs(
    ...
    let nextFightClick$ = fromEvent(live.getElement(CLASS_NAMES.BUTTONS.NEXT_FIGHT), "click")
        .pipe(
            startWith(null)
        );
    return zip(range(0, fightCard.fights.length), nextFightClick$).pipe(
        map(([indexOfFight, _]) => indexOfFight),
    ...
    return zip(getFightTickObs(live), getFightEventObs(live))
        .pipe(
            ...
            finalize(() => {
                ...
                live.showElement(CLASS_NAMES.BUTTONS.NEXT_FIGHT);
            })
    );
}
