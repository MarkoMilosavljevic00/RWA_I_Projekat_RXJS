...
import { CLASS_NAMES, RULES, TYPE_OF_ELEMENTS } from "../utilities/constants";
...
export class PickerComponent extends Component {
    ...
    constructor(fightCard: FightCard) {
        ...
        this.container = selectElementByClass(
            document.body,
            CLASS_NAMES.CONTAINERS.PICKER
        );
        this.fightCard = fightCard;
        setSelectsOptionsFromValues(
            this.container,
            CLASS_NAMES.SELECTS.WEIGHTCLASS,
            CLASS_NAMES.OPTIONS.WEIGHTCLASS,
            Weightclass
        );
        setSelectsOptionsFromValues(
            this.container,
            CLASS_NAMES.SELECTS.RULES,
            CLASS_NAMES.OPTIONS.RULES,
            Rules
        );
    }
    ...
    setFighter(fighter: Fighter, corner: Corner) {
        ...
        if (corner == Corner.Red) {
            ...
            cornerImg = selectElementByClass(
                this.container,
                CLASS_NAMES.IMAGES.RED_CORNER
            ) as HTMLImageElement;
            winnerLabel = selectElementByClass(
                this.container,
                CLASS_NAMES.LABELS.RED_CORNER_WINNER
            ) as HTMLLabelElement;
            ...
            winnerLabel = selectElementByClass(
                this.container,
                CLASS_NAMES.LABELS.BLUE_CORNER_WINNER
            ) as HTMLLabelElement;
    ...
    addFight(newFight: Fight) {
        ...
        if (message !== Message.Success) return showError(message);

        let fightListDiv = selectElementByClass(
            this.container,
            CLASS_NAMES.LISTS.FIGHT
        );
        let fightTempDiv = selectElementByClass(
            this.container,
            CLASS_NAMES.TEMPLATES.FIGHT
        );
        ...
        fightItem.classList.add(
            `${CLASS_NAMES.ITEMS.FIGHT + this.numberOfFights}`
        );
    ...
    setSelectOptionsForRounds(rule: Rules) {
        let numberOfRounds = RULES.NUMBER_OF_ROUNDS[rule];
        setSelectOptionsToNumber(
            this.container,
            CLASS_NAMES.SELECTS.ROUND,
            CLASS_NAMES.OPTIONS.ROUND,
            numberOfRounds
        );
    }
    ...
    setSelectOptionsForMethods(rule: Rules) {
        ...
        setSelectsOptionsFromValues(
            this.container,
            CLASS_NAMES.SELECTS.METHOD,
            CLASS_NAMES.OPTIONS.METHOD,
            methods
        );
    }
    ...
    getFightInfo(): Fight {
        ...
        let winner = mapStringToEnum<Corner>(
            getCheckedRadioValue(CLASS_NAMES.RADIO.WINNER),
            Corner
        );
        let method = mapStringToEnum<Method>(
            getSelectedValue(
        ...
        let round;
        if (method === Method.Decision) round = 0;
        else
            round = parseInt(
                getSelectedValue(
                    this.container as HTMLDivElement,
    ...
    renderFightInformation(fightDiv: HTMLElement, newFight: Fight) {
        ...
        let redCornerImg = selectElementByClass(
            fightDiv,
            CLASS_NAMES.IMAGES.RED_CORNER
        ) as HTMLImageElement;
        let blueCornerImg = selectElementByClass(
            fightDiv,
            CLASS_NAMES.IMAGES.BLUE_CORNER
        ) as HTMLImageElement;
        let redCornerLabel = selectElementByClass(
            fightDiv,
            CLASS_NAMES.LABELS.RED_CORNER
        );
        let blueCornerLabel = selectElementByClass(
            fightDiv,
            CLASS_NAMES.LABELS.BLUE_CORNER
        );
        let winnerLabel = selectElementByClass(
            fightDiv,
            CLASS_NAMES.LABELS.WINNER
        );
        let methodLabel = selectElementByClass(
            fightDiv,
            CLASS_NAMES.LABELS.METHOD
        );
        let roundLabel = selectElementByClass(
            fightDiv,
            CLASS_NAMES.LABELS.ROUND
        );
        let redCornerOddLabel = selectElementByClass(
            fightDiv,
            CLASS_NAMES.LABELS.RED_CORNER_ODD
        );
        let blueCornerOddLabel = selectElementByClass(
            fightDiv,
            CLASS_NAMES.LABELS.BLUE_CORNER_ODD
        );
        let pickDiv = selectElementByClass(fightDiv, CLASS_NAMES.DIVS.PICK);

        rulesLabel.innerHTML = newFight.rules;
        weightclassLabel.innerHTML = newFight.redCorner.weightclass;
        redCornerImg.src = `${
            PATHS.IMAGES.FIGHTERS + newFight.redCorner.pictureSrc
        }`;
        blueCornerImg.src = `${
            PATHS.IMAGES.FIGHTERS + newFight.blueCorner.pictureSrc
        }`;
        ...
        } else roundLabel.innerHTML = `Round: ${newFight.yourPick.round}`;
        ...
        else pickDiv.classList.add(CLASS_NAMES.STYLES.BLUE_TEXT);
    }

    renderUndoButton() {
        let undoButton = selectElementByClass(
            this.container,
            CLASS_NAMES.BUTTONS.UNDO
        );
        if (this.checkIsFightCardEmpty())
            undoButton.classList.add(CLASS_NAMES.STATES.COLLAPSE);
        else undoButton.classList.remove(CLASS_NAMES.STATES.COLLAPSE);
    }

    removeFight(fightIndex: number = this.numberOfFights - 1) {
        this.fightCard.removeFight(fightIndex);
        let fightDiv = selectElementByClass(
            this.container,
            CLASS_NAMES.ITEMS.FIGHT + fightIndex
        );
        fightDiv.remove();
    ...
    updateFightDivsIds(fightIndex: number, partOfClassName: string) {
        let fightDivs = selectElementsByPartialClass(
            this.container,
            partOfClassName
        );
        fightDivs.forEach((fightDiv) => {
            const classes = fightDiv.className.split(" ");
            const fightClasses = classes.filter((className) =>
                className.startsWith(partOfClassName)
            );
            if (fightClasses.length === 1) {
                const currentFightIndex = parseInt(
                    fightClasses[0].substring(partOfClassName.length),
                    10
                );
                if (currentFightIndex >= fightIndex) {
                    fightDiv.classList.remove(
                        `${partOfClassName}${currentFightIndex}`
                    );
                    const newFightIndex = currentFightIndex - 1;
                    fightDiv.classList.add(
                        `${partOfClassName}${newFightIndex}`
                    );
                }
            }
        });
